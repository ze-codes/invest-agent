...
def compute_weekly_net_settlements(db: Session, weeks_back: int = 12) -> List[Dict[str, Any]]:
    ...
    issues = get_latest_series_points(db, "UST_AUCTION_ISSUES", limit=weeks_back * 20)
    redemptions = get_latest_series_points(db, "UST_REDEMPTIONS", limit=weeks_back * 20)
    interest = get_latest_series_points(db, "UST_INTEREST", limit=weeks_back * 20)
...
def upsert_weekly_net_settlements(db: Session, *, series_id: str = "UST_NET_SETTLE_W", weeks_back: int = 26) -> int:
